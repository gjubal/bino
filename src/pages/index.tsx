...
const Home: NextPage = () => {
	...
	const last100Transactions = trpc.useQuery(['transactions.getLast100Transactions']);

	if(!last100Transactions.data || last100Transactions.error) {
		return <div>{last100Transactions.error?.message}</div>;
	}
	...
	return (
		<>
			...
			{/* <LoginComponent /> */}
			<div>
				{last100Transactions.data.map(transaction => (
					<p key={transaction.transaction_id}>{transaction.name} - ${transaction.amount} - {transaction.date}</p>
				))}
			</div>
		</>
	);
...
const LoginComponent = () => {
	...
	const onSubmit: SubmitHandler<AuthenticateUserInput> = useCallback(
